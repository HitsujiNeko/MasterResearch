...
**2行目（建物はあるが有効高さを持つ建物が1棟も無いセル）に該当するセルは、`var = -1` の 149 棟だけを含むセルであり、新方式でも重なり方式・重心方式のいずれでも高さが得られないため NULL のまま残る。** 旧方式の実測（30m 26 セル・90m 6 セル・300m 3 セル）は「重心を持つ建物が全て無効高さ」のセルのみを数えていたが、新方式の該当カテゴリには「無効高さの建物だけが張り出したセル」も加わるため、件数が変わりうる（無効高さは ROI 全体で 149 棟のみのため、規模は小さい）。
...
**脱落の解消**: LST有効セルのうち建物高さNULLで脱落するセル数は **53,784セル → 6セルへ99.99%解消**した。残る6セルは有効高さを持つ建物が1棟も無いセルという既知の例外に該当する（3.4節）。

**この6セルの測定条件（重要）**: 上記はセルの母集団に `dataset_limited_20230707_032329_hanoi_30m.gpkg` のLST有効セルを用い、高さの値は本節で再算出した新方式の `build_gba.gpkg` から `cell_id` で読み直して補完規則を適用した値である。**同分析用データセット自体を新方式で再生成した値ではない**（同ファイルは2026-08-25生成で、本節の再算出（2026-08-27）より前のものである）。すなわち6セルは「032329のデータセットを新方式で作り直した場合にこうなる」という見込み値であり、実在のファイルを測った値ではない。

**追試の条件**: 上記の6セルは次のSQLで再現できる（2026-09-01に再測定して6セルを確認済み。GeoPackageはSQLiteのため直接照会できる）。

```sql
ATTACH DATABASE 'data/output/params/hanoi/30m/build_gba.gpkg' AS p;
SELECT COUNT(*) FROM limited_20230707_032329 d
JOIN p.build_gba b ON b.cell_id = d.cell_id
WHERE d.IN_ANALYSIS_AREA = 1 AND d.LST IS NOT NULL AND d.LST_VALID_RATIO >= 0.5
  AND b.BUILD_H_MEAN IS NULL AND NOT (b.BUILD_COV = 0 AND b.BUILD_DEN = 0);
```

同種の脱落セル数は測定対象によって次のように異なる。**名称が同じでも母集団・方式・パラメータテーブルの版が違うため、突き合わせないこと。**

| セル数 | 測定対象 | 方式 | LST有効セル（母集団） | 再測定 |
|---|---|---|---|---|
| 53,717 | `dataset_limited_20230707_032329_hanoi_30m.gpkg`（実在のファイル） | 旧方式 | 1,957,204 | 確認済 |
| 53,784 | 同上のセル母集団に旧方式のパラメータテーブルを適用（版は未特定[^oldtable]） | 旧方式 | 1,957,204 | 不可 |
| **6** | 同上のセル母集団に再算出後のパラメータテーブルを適用 | **新方式** | 1,957,204 | 確認済 |
| 21 | `dataset_limited_20230707_032305_hanoi_30m.gpkg`（実在のファイル） | 新方式 | 3,337,319 | 確認済 |

[^oldtable]: 旧方式のパラメータテーブルは本節の再算出で上書きされているため再測定できず、53,784 がどの版で測られたかは特定できない。53,717 との差（67セル）はテーブルの版の違いによる想定内の差として扱う。

**6 と 21 は直接比較できない。** 観測シーンが異なり、LST有効セルの母集団（1,957,204 対 3,337,319）も空間分布も違ううえ、6 は 032329 のセル母集団へ新方式のパラメータテーブルを適用した値、21 は 032305 の分析用データセットの実測値である。**母集団の広さの違い（1.71倍）だけではセル数の差（3.5倍）を説明できない。** 読めるのは、いずれの新方式の測定でも旧方式の53,717／53,784セルに対して2桁以上小さいという向きだけである。各数値の分析側からの読み方は、観測 20230707_032305（21セル）が [limited_analysis_results.md](../../03_results/limited_analysis_results.md) 1.2節・3.8節、観測 20230707_032329（53,717セル）が [limited_analysis_results_20230707_032329.md](../../04_archive/limited_analysis_results_20230707_032329.md) 3.8節にある。
