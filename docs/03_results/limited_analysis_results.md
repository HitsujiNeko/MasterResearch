...
- 感度分析（都市化セル限定）では NDBI の重要度がさらに支配的になり（RF Importance 0.581→0.777）、NDVI・NDWI・ELEV_MEAN の相対的な寄与が下がる。これは都市化セルに限定すると植生・水域・標高の勾配が失われるため、自然な傾向と解釈できる
- Spatial CV の R²（RF）は主結果 0.7801 → 感度分析 0.7233 に低下する。有効域全体（農地・水域を含む）の方が、都市化セルのみより説明力が高い
...
4.5節の比較から、この判断により Spatial CV R²（RF）は0.7801（主結果）対0.7233（都市化セル限定）と、有効域全体を対象にした方が高い説明力を示すことが確認できた。これは「Limitedシナリオが農地・水域を含む多様な土地被覆下でLSTをどれだけ説明できるか」という本Issueの評価意図と整合する。
