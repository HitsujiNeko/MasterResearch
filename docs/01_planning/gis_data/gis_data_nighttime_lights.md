...
**解析グリッド（30m）での影響（2026-08-30 確認）**: 上記の有効画素率は約464m画素についての値であり、**ROIポリゴン内部の画素に欠測が無い**ことを示す。一方、解析グリッドでは**ROI境界に欠測の帯が生じる**。配布ラスタをROIでクリップしているため、ROI境界をまたぐ画素が無効値となり、その画素へ入る30mセルは有効画素を1つも持たないためである。ROI全域（3,736,107セル）で11,905セルが該当し、いずれもROI境界から464m以内（実測の最大は288m）にある。陸地であり水域被覆はほぼ0（平均0.070）である。

したがって「ROI全域に欠測なし」と「解析グリッドに欠測がある」は矛盾しない。前者はROI内部の画素、後者はROI境界をまたぐグリッドセルについての記述である。同じ構造は人口（LandScan 約920m画素）でも生じ、画素が粗いほど帯は広くなる（[gis_data_population.md](gis_data_population.md) 6.4節）。
