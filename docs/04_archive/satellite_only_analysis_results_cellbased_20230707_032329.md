# Satellite Only 分析結果（観測 20230707_032329・cell_id結合経路・アーカイブ）

**最終更新**: 2026-09-02  
**関連ドキュメント**: [satellite_only_analysis_results_cellbased.md](../03_results/satellite_only_analysis_results_cellbased.md), [satellite_only_analysis_results.md](../03_results/satellite_only_analysis_results.md), [analysis_rq3_satellite_only_guide.md](../02_methods/analysis_rq3_satellite_only_guide.md), [calc_urban_params_io_spec.md](../02_methods/calc_urban_params/calc_urban_params_io_spec.md) 6.7節, [analysis_workflow.md](../02_methods/analysis_workflow.md), [research_guide.md](../01_planning/research_guide.md)  
...
**本ドキュメントは観測 `2023-07-07T03:23:29Z`（以下 032329）に基づく記録であり、現行の結果ではない。** 現行の Satellite Only シナリオ（cell_id結合経路）の結果は [satellite_only_analysis_results_cellbased.md](../03_results/satellite_only_analysis_results_cellbased.md)（観測 `2023-07-07T03:23:05Z`）を正本とする。

### 0.1 アーカイブへ移した理由

Limited シナリオを観測 032305 で再構築したことに伴い、シナリオ間比較を同一観測のうえで行うため Satellite Only も 032305 へ移行した。**観測が異なる2つのランは、母数（有効カバレッジ）から抽出される標本まで別物であり、数値を直接比較できない。**

| 軸 | 本ドキュメント（032329） | 現行（032305） |
|---|---|---|
| 観測 | `2023-07-07T03:23:29Z` | `2023-07-07T03:23:05Z` |
| フィルタ後母数 | 1,955,963セル | 3,317,272セル |
| フィルタ脱落診断（`filter_dropout`） | 出力に無い（診断の常設化より前のラン） | 出力にある |

処理条件（`LST_VALID_RATIO >= 0.5`・10万件サンプル・`block_size_m=2,700`・5-fold・RF 300本）は両ランで同一である。**それでも母数が異なる以上、R² や変数重要度の差を観測の違いによる効果として読んではならない。**

### 0.2 現行ドキュメントとの対応

| 本ドキュメントの節 | 現行ドキュメントでの扱い |
|---|---|
| 3節（データと処理条件） | 032305 の値で書き直し |
| 4.1〜4.4節（モデル性能・変数重要度・SHAP） | 032305 の値で書き直し |
| 4.5節（ブロック境界の影響概算） | 032305 の母集団で再測定 |
| 4.6節（移植の等価性確認・ゴールデン比較） | **移していない**（0.3節） |
| 5節（結果の解釈）・6節（今後の研究の方向性） | 032305 の結果に沿って書き直し |

**軸としての対応であり、数値は対応しない。**

### 0.3 本ドキュメントにのみ残る内容

**4.6節（移植の等価性確認）は本ドキュメントにのみ残る。** 旧ピクセル経路が出力した 032329 のサンプルCSVを入力とした一度きりの確認であり、032305 では同じ入力が存在しないため再現できない。関数単位の移植が数値的に等価であったことを示す記録として保持する。

---

### 0.4 旧ピクセル経路との関係（必読）

本ドキュメントは、既存の [satellite_only_analysis_results.md](../03_results/satellite_only_analysis_results.md)（旧経路・ピクセル単位・3観測日）とは**別のパイプラインによる別物の結果**である。**数値を直接比較しない。**
...
30m・単一観測日でも、**衛星指標だけでLSTを一定程度説明できる**。Random Forest の random split `R²=0.7427`、Spatial CV `R² mean=0.7277` であった。旧経路とは格子、観測日数、Spatial CV の定義が異なるため（0節）、結果の比較は行わない。
