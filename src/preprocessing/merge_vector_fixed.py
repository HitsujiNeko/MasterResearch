...
def has_dgnv8_support(ogrinfo_path: str) -> bool:
    ...
    try:
        result = subprocess.run(
            [ogrinfo_path, "--formats"], capture_output=True, text=True, check=True
        )
    except Exception:
        return False
    ...
    for path in candidates:
        try:
            ...
            if require_dgnv8 and not dgnv8_supported:
                logger.warning("ogr2ogr candidate skipped because DGNv8 is unavailable: %s", path)
                continue
...
def check_file_validity(dgn_file: Path, ogr2ogr_path: str) -> dict:
    ...
    try:
        ...
        result = subprocess.run(
            [ogrinfo_path, str(dgn_file)], capture_output=True, text=True, timeout=10
        )

        if result.returncode == 0:
            # レイヤー数をカウント
            layer_count = result.stdout.count("Layer name:")
            return {"valid": True, "layer_count": layer_count, "error_message": None}
        else:
            return {"valid": False, "layer_count": 0, "error_message": result.stderr}
    except Exception as e:
        return {"valid": False, "layer_count": 0, "error_message": str(e)}


def merge_to_geopackage(
    input_dir: Path, suffix: str, output_root: Path, ogr2ogr_path: str, skip_existing: bool = False
) -> dict:
...
def main():
    ...
    parser.add_argument(
        "--root", type=Path, default=Path(r"整備データ"), help="Vector_* があるルートディレクトリ"
    )
    parser.add_argument("--suffix", choices=SUFFIXES, help="特定種類のみ統合 (例: DC)")
    parser.add_argument("--skip-existing", action="store_true", help="既存のGPKGファイルをスキップ")
    parser.add_argument(
        "--output", type=Path, default=Path(r"整備データ/merge"), help="出力先ディレクトリ"
    )
    ...
    ogr2ogr_path = get_ogr2ogr_path(
        require_dgnv8=("TV" in ([args.suffix] if args.suffix else SUFFIXES))
    )
    ...
    for s in targets:
        ...
        stats = merge_to_geopackage(in_dir, s, args.output, ogr2ogr_path, args.skip_existing)
