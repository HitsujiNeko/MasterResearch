...
| [limited_analysis_results.md](03_results/limited_analysis_results.md) | Limited シナリオ分析結果（cell_id結合） | 分析ランの台帳（1ラン=1行）、ラン間で共通の処理条件、モデル性能・変数重要度・多重共線性をランを横断して整理。建物高さ構成軸（both/mean/max/pc1）の比較と採用構成の根拠を含む。フル表は貼らず CSV/JSON を数値の正本とする | `src/analysis/analysis_rq3_limited.py` |
