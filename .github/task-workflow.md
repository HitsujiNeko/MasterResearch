...
**判定の前提**: 判定基準は「現在チェックアウトされているブランチに、このセッション自身が会話内で着手した記録を持つか」であり、ブランチ名の一致・不一致だけでは判定しない。会話履歴が空の新規セッションは、現在チェックアウトされているブランチに関する着手記録を一切持たないため、ブランチ名にかかわらず「着手記録なし」として扱う。

- **着手記録がある場合**（このセッション自身が現在のブランチの Issue に継続して取り組んでいる）: 下表に従う
...
- **着手記録がない場合**（セッション開始直後。現在チェックアウトされているブランチが存在するなら、それは必ず別セッションの作業）: ブランチ名にかかわらず並列実行と判定し、[parallel-workflow.md](./parallel-workflow.md) の「別セッションで並列作業を開始した場合（方式 B）」に従って、このセッション自身を worktree で隔離してユーザーが指示した Issue に着手する。指示された Issue の作業を他のセッションに委ねてはならない。
