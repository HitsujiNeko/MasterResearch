...
class TestComputeCellAreaKm2:
    ...
    def test_matches_known_area_at_equator(self) -> None:
        ...
        pixel_size_deg = 1.0 / 120.0  # 30 arc-sec
        transform = from_origin(0.0, pixel_size_deg / 2, pixel_size_deg, pixel_size_deg)

        areas = target.compute_cell_area_km2(transform, height=1, width=1)
    ...
    def test_area_decreases_toward_the_pole(self) -> None:
        ...
        areas = target.compute_cell_area_km2(transform, height=100, width=10)
    ...
    def test_hanoi_cell_area_is_smaller_than_equator(self) -> None:
        ...
        pixel_size_deg = 1.0 / 120.0
        equator = target.compute_cell_area_km2(
            from_origin(0.0, pixel_size_deg / 2, pixel_size_deg, pixel_size_deg), 1, 1
        )
        hanoi = target.compute_cell_area_km2(
    ...
    def test_rejects_non_positive_dimensions(self) -> None:
        ...
        with pytest.raises(ValueError):
            target.compute_cell_area_km2(transform, height=0, width=10)
    ...
    def test_rejects_rotated_grid(self) -> None:
        ...
        with pytest.raises(ValueError, match="回転・スキュー"):
            target.compute_cell_area_km2(rotated_transform, height=10, width=10)
...
class TestComputeDensityArray:
    ...
    def test_divides_count_by_cell_area(self) -> None:
        ...
        density = target.compute_density_array(count_array, cell_area_km2)
    ...
    def test_keeps_nodata_pixels_as_nodata(self) -> None:
        """nodata 画素は密度側でも nodata のまま残す（0 人と混同しないため）。"""
        count_array = np.array([[target.OUTPUT_NODATA, 5.0]], dtype=np.float32)
        cell_area_km2 = np.array([[0.5]])

        density = target.compute_density_array(count_array, cell_area_km2)

        assert density[0, 0] == target.OUTPUT_NODATA
        assert density[0, 1] == pytest.approx(10.0)

    def test_zero_population_stays_zero(self) -> None:
        """人口 0 の画素は密度 0 になり、nodata にはならない。"""
    ...
    def test_returns_float32(self) -> None:
        ...
        density = target.compute_density_array(count_array, np.array([[0.5]]))
