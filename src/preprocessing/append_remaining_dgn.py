...
def append_remaining_files(
    ...
    for i, dgn_file in enumerate(remaining_files, 1):
        ...
        try:
            subprocess.run(
                cmd_append, capture_output=True, text=True, check=True, timeout=timeout_seconds
            )
            logger.info("  ✅ 追加成功")
            ...
            logger.warning("     このファイルをスキップリストに追加することを推奨")
            stats["skipped_files"] += 1
            stats["errors"].append(
                {"file": dgn_file.name, "error": f"Timeout ({timeout_seconds}s)"}
            )
            ...
            stats["errors"].append({"file": dgn_file.name, "error": error_msg})
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="既存GPKGに残りのDGNファイルを追加")
    parser.add_argument(
        "suffix", choices=["CS", "DC", "DH", "GT", "RG", "TH", "TV"], help="データ種類（例: DC）"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument("--skip", nargs="+", help="スキップするファイル名のリスト")
    parser.add_argument(
        "--timeout", type=int, default=180, help="各ファイルのタイムアウト秒数（デフォルト: 180秒）"
    )
    ...
    stats = append_remaining_files(
        input_dir, suffix, output_gpkg, ogr2ogr_path, skip_files, processed_files, args.timeout
    )
