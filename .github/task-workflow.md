...
**判定の前提**: 現在のブランチが `main` の場合は常に単一実行として扱う。`main` 以外の場合は「そのブランチに、このセッション自身が会話内で着手した記録を持つか」で判定し、ブランチ名の一致・不一致だけでは判定しない。

- **現在のブランチが `main` の場合**: 単一実行 → 以降の標準ワークフローに従う
- **現在のブランチが `main` 以外で、着手記録がある場合**（このセッション自身が現在のブランチの Issue に継続して取り組んでいる）: 下表に従う

  | 現在のブランチ | ユーザーの指示 | 判定 |
  |---|---|---|
  | `XX/...`（指示と同じ Issue） | 「#XX に取り掛かります」 | 単一実行（継続） → 以降の標準ワークフローに従う |
  | `YY/...`（指示と別の Issue） | 「#XX に取り掛かります」 | 並列実行 → [parallel-workflow.md](./parallel-workflow.md) の「セッション中に並列指示を受けた場合」に従う |

- **現在のブランチが `main` 以外で、着手記録がない場合**（会話履歴が空の新規セッションが、他セッションの作業ブランチに遭遇している）: ブランチ名にかかわらず並列実行と判定し、[parallel-workflow.md](./parallel-workflow.md) の「別セッションで並列作業を開始した場合（方式 B）」に従って、このセッション自身を worktree で隔離してユーザーが指示した Issue に着手する。指示された Issue の作業を他のセッションに委ねてはならない。

セッション中にユーザーから「#XX を並行で進めたい」と指示を受けた場合も、上記の前提に従って判定し、該当するフローに従う。
...
計画書はローカル作業用ファイルであり、コミット・push の対象外とする。タスク完了時にローカルから削除する。
