...
**解析グリッド（30m）での影響（2026-08-30 確認）**: 上記の有効画素率は約460m画素（緯度補正後の実測値。赤道換算の単純計算では約464mになるが、ハノイの緯度で補正すると460.6m）についての値であり、**ROIポリゴン内部の画素に欠測が無い**ことを示す。一方、解析グリッドでは**ROI境界に欠測の帯が生じる**。配布ラスタをROIでクリップしているため、ROI境界をまたぐ画素が無効値となり、その画素へ入る30mセルは有効画素を1つも持たないためである。ROI全域（3,736,107セル）で11,905セルが該当し、いずれもROI境界から460m以内（実測の最大は288m）にある。陸地であり水域被覆はほぼ0（平均0.070）である。
