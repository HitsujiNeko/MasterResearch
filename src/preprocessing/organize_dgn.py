"""
DGNファイルを種類別ディレクトリへ振り分ける前処理スクリプト。

ソースディレクトリ配下の領域フォルダから、ファイル名末尾のサフィックス
（CS/DC/DH/GT/RG/TH/TV）に応じてDGNファイルを抽出し、ターゲットディレクトリ
配下の `Vector_<サフィックス>` フォルダへコピーする。

本スクリプトの出力（`Vector_*` ディレクトリ）は、merge_vector_fixed.py の
入力として使用する。
"""
...
def find_dgn_files(source_root: Path) -> Iterator[tuple[str, Path]]:
...
def ensure_targets(target_root: Path) -> dict[str, Path]:
    targets = {}
    for suffix in SUFFIXES:
