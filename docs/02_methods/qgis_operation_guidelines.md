...
ROI固定の単発描画が安全なのは、描画対象がROI内に限られ、全件の同時描画が発生しないためである。**外れ値ジオメトリによりレイヤーのextentが破綻するデータ**（測量データにダミー座標`499999.1`等が含まれる例）でも、描画範囲をROIに固定すればROI内の実データのみが描画され、無害化できることを確認している。
...
カテゴリ値ラスタの公式配色は、一次情報がSTACメタデータに無い場合がある。以下の順序で確認し、**記憶や一般に流布した値で埋めない**。

1. **コレクションのmetadata**を確認する
2. **ソースCOGに埋め込まれたカラーテーブル**を確認する。署名付きURLで`rasterio.open()`し、ヘッダーのみ読めば全体をダウンロードせずに実測できる
