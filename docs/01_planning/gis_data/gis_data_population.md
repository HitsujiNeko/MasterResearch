...
| 指標 | セルあたり人口 | 密度（人/km²） |
|---|---|---|
| Pearson r | 0.8130 | 0.8130 |
| Spearman ρ | 0.6920 | 0.6921 |
| 平均バイアス（WorldPop − LandScan） | +23.3 | +29.2 |
| 中央値バイアス | +369.1 | +461.6 |
| 平均絶対誤差 | 1,339.1 | 1,675.1 |
| RMSE | 2,916.0 | 3,647.8 |

ROI全体の総人口比は1.017（WorldPopが1.7%多い）で、集計レベルではよく一致する。一方、**セル単位では平均バイアスがほぼ0（+23人）なのに中央値バイアスが大きく正（+369人）**である。これは「大多数のセルではWorldPopの方が多く、少数の都心セルではLandScanが大きく上回って相殺している」ことを意味し、実効人口（昼間の人口移動を考慮）が都心へ集中する概念差と整合する。QGIS表示でも、LandScanは中心部の高密度ブロックが集中的かつ急峻で、WorldPopは中心から連続的に減衰する分布を示す。
