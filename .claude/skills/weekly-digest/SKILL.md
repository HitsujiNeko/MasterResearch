...
1. **Project 全アイテムの取得**（GraphQL。`gh project item-list` は日本語フィールド名を破壊するため使用禁止）:

   ```bash
   gh api graphql -f query='query { user(login:"HitsujiNeko") { projectV2(number:1) { items(first:100) { nodes { content { ... on Issue { number title state labels(first:10){ nodes { name } } } } fieldValues(first:20) { nodes { ... on ProjectV2ItemFieldDateValue { date field { ... on ProjectV2FieldCommon { name } } } ... on ProjectV2ItemFieldSingleSelectValue { name field { ... on ProjectV2FieldCommon { name } } } } } } } } } }'
   ```
