...
本研究では衛星由来指標としてNDWI（水域・湿潤域指標）と水域率（`WATER_RATIO`、NDWI閾値ベース）を既に算出しているが、これらは連続値・単一時点の指標である。都市構造パラメータとしては、以下の2種類の水域関連指標が求められる。

- **水域近接距離**（`WATER_DIST`等）: 最近接の水域までの距離。先行研究S3・S5・S6で扱われる`PW`（距離変数）に相当。
- **水域面積率**（`WATER_COV`）: グリッド内の水域面積割合。`analysis_workflow.md`のグループB指標に位置づけられる。
