...
なお誤差分散は高密度な中心部での実測であり、低被覆セルが多い ROI 全域では誤差が小さくなるため、上記の λ は保守的（悪い側）な見積りである。また GBA フットプリント自体の過検出・未検出の誤差は未定量であり、30m セルでは建物1棟の検出漏れが被覆率を 0.11 動かすため、データ側の誤差が離散化誤差を上回る可能性がある。
