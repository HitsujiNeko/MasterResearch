...
- セッション開始時に Claude が現在のステータスを確認し、未着手なら「進行中」に更新・開始日を設定して `main` から作業ブランチ `{Issue番号}/{タスク要約英文}` を作成、保留中なら確認してから更新する
- 保留時は Claude が理由と再開条件を Issue にコメントし、ステータスを「保留」に更新する
- **タスク完了時はClaudeがコミット・push・PR作成まで行う**。ユーザーはレビュー・動作確認とPRのsquash mergeを行い、「マージしました」と伝えるとClaudeがmain復帰・作業ブランチ削除・prompt削除・完了処理を行う
