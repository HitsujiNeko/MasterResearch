...
**CRSの判定はQGISではなくrasterio / GeoPandas側で行い、QGISは描画確認（ROIと重なるか・位置がずれていないか）に限定する。**

```python
import geopandas as gpd
import rasterio

# ベクタ（Shapefile・GeoPackage 等）
print(gpd.read_file(vector_path).crs)
...
本研究のGBA建物データ（約307万件）や測量データ（数十万件規模のレイヤーが複数）を**対話キャンバスで同時に可視化状態にする**と、QGIS自体がクラッシュする事例が確認されている。**一方、描画範囲をROIに固定した単発描画（`render_map`・印刷レイアウトexport）は同規模のデータでも安全**である。切り分けの詳細は[qgis_operation_guidelines.md](qgis_operation_guidelines.md)の「大規模レイヤーの取り扱い」を参照。
