...
- 命名規則は既存の `lulc_glc_fcs30d.qml` に倣い `{カテゴリ}_{データセット}.qml`（英小文字スネークケース）とする
...
- QGIS-MCP で取得データを ROI と重ねて表示し、`inspect-gis-data` の `get_canvas_screenshot` で取得する
- ROI 基準でズームし、カバレッジ・CRS 整合・分布/外れ値が判読できる構図とする
- 原則 `get_canvas_screenshot`（容量抑制）。高解像度 `render_map` は必要時のみ
- 大規模レイヤは既存のクラッシュ対策（ROI 基準ズーム・こまめ保存）に従う
