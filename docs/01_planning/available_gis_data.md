...
- 標高データは `FABDEM v1.2`（Copernicus DEM 派生の準DTM）を `Limited` シナリオの採用DEMとして確定した（BSHorizon との比較で全指標最良）。標高パラメータは `ELEV_MEAN`（m）の1種で、DEMラスタを coarse グリッドへ平均再投影して算出する。**`ELEV_MEAN` は品質管理列 `VALID_GIS_MASK` の判定材料に含めない**（標高は連続量であり `0` は欠損ではなく海抜0mを意味するため）。ライセンスは CC BY-NC-SA 4.0（非商用）で、論文・資料への帰属文記載が必須である点に注意を要する（詳細は [gis_data_dem.md](gis_data/gis_data_dem.md) Section 7）。
