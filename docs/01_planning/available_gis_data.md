...
**最終更新**: 2026-08-27  
**関連ドキュメント**: [research_guide.md](research_guide.md), [urban_structure_parameters.md](urban_structure_parameters.md), [analysis_workflow.md](../02_methods/analysis_workflow.md), [calc_urban_params_guide.md](../02_methods/calc_urban_params_guide.md), [CodingRule.md](../02_methods/CodingRule.md)  
...
- `Limited` シナリオの建物データソースは GBA に移行済み（`config.py`）。Microsoft 時代のカバレッジ欠落問題は解消された。建物パラメータは `BUILD_COV`（被覆率）・`BUILD_DEN`（棟/ha）・`BUILD_H_MEAN` / `BUILD_H_MAX`（m）の4種で算出方法を確定済み。棟数密度は重心が属するセルへ帰属させる重心方式、被覆率は fine グリッドへのラスタ化と平均集約による。高さ（`BUILD_H_MEAN` / `BUILD_H_MAX`）は重心方式単独から**重なりベースと重心方式の併用**へ 2026-08 に変更した（大規模建物の張り出しセルが NULL になり分析側で系統的に脱落していたため）。**被覆率は 30m スケールでは離散化により10段階の値しか取らず、回帰係数が約2割縮む見積り**である点に解釈上の注意を要する。高さも新方式で同水準の離散化誤差（30m 希釈係数 0.968）を新たに持つ（詳細は [gis_data_buildings.md](gis_data/gis_data_buildings.md) Section 3）。
- 標高データは `FABDEM v1.2`（Copernicus DEM 派生の準DTM）を `Limited` シナリオの採用DEMとして確定した（BSHorizon との比較で全指標最良）。標高パラメータは `ELEV_MEAN`（m）と `ELEV_VALID_RATIO`（セル内のDEM有効画素率, 0-1）の2種で、DEMラスタを coarse グリッドへ平均再投影して算出する。平均はセル内の有効画素のみで取るため、部分被覆セルの判別には `ELEV_VALID_RATIO` を用いる（`NaN` の件数だけでは捕捉できず、有効カバレッジを過大評価する）。**いずれも品質管理列 `VALID_GIS_MASK` の判定材料に含めない**（標高は連続量であり `0` は欠損ではなく海抜0mを意味するため。有効画素率も「DEMが覆っているか」という別軸の指標であるため）。ライセンスは CC BY-NC-SA 4.0（非商用）で、論文・資料への帰属文記載が必須である点に注意を要する（詳細は [gis_data_dem.md](gis_data/gis_data_dem.md) Section 7）。
- 土地利用・人口密度・夜間光・水域（近接距離・面積率）・POI密度・不透水面率・公園近接距離の7カテゴリについて、オープンソースデータセット候補の調査を完了した。各カテゴリの候補比較・推奨方針は Section 4 のカテゴリ別詳細ドキュメントを参照。**Hanoi ROI での実データを取得済みなのは土地利用・人口密度・夜間光の3カテゴリ**で、水域・POI密度・不透水面率・公園近接距離は未取得である。説明変数としての採否は [urban_structure_parameters.md](urban_structure_parameters.md) を正本とし、本資料では扱わない。
