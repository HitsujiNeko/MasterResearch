...
検証:
    ...
    ``BUILD_H_MEAN`` / ``BUILD_H_MAX`` は本変更により、現行では QGIS と
    1.0e-6 m で厳密一致していた検証結果が成り立たなくなる（重なり方式が
    fine解像度による離散化誤差を新規に持ち込むため）。新方式でのQGIS突合
    結果・希釈係数の実測は本Issueの動作確認手順で実施中であり、確定次第
    本節を更新する。
