...
- 植生（植生被覆率）: 土地被覆ラスタまたは OSM 土地利用ポリゴン（入力源未確定。6.4節参照）
...
- 植生（TV / OSM landuse / 土地被覆ラスタ等）
  - `GREEN_COV`: 植生被覆率
- 土地被覆クラス別面積率（土地被覆ラスタ）
  - **カテゴリカルラスタ**のため、連続量を対象とする `Resampling.average` は使えない。クラスごとの画素割合を集約する処理を別途設計する
- 人口密度・夜間光強度（ラスタ）
  - いずれも連続量ラスタのため、標高と同じくラスタ集約経路（Step C と共通の集約関数）を用いる
  - **入力ラスタの解像度が解析スケールより粗い**ため、細スケールの値は実質的な内挿になる。解釈上の制約は [urban_structure_parameters.md](../01_planning/urban_structure_parameters.md) §2.1 を参照
- 測量GIS由来の標高（DH）
