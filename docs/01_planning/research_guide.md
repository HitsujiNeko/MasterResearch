...
本研究で用いるデータは、入手可能性の観点から3層に整理される。この3層構造こそが、RQ3で比較する3つの分析シナリオに対応しており、本研究の設計の骨子をなす。

| データの層 | 位置づけ | 対応するシナリオ |
|---|---|---|
| **衛星由来データ** | 目的変数であるLSTと、衛星由来の都市構造指標。全球で一貫して入手可能 | `Satellite Only` |
| **公開GISデータ** | 全球・広域で公開され、測量データを持たない都市でも入手可能な都市構造データ | `Limited` |
| **測量GISデータ** | 現地測量に由来する高精度データ。利用可能地域が限定的 | `Full` |

シナリオの定義は [CLAUDE.md](../../CLAUDE.md) の最小用語集を正本とする。
