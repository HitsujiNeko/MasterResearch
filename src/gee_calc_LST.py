...
CSVファイルの例は以下の通り：
roi_shapefile_path,start_date,end_date,cloud_threshold,valid_pixel_threshold,output_epsg,lst_method,gee_project_id
data\GISData\ROI\hanoi_roi.shp,2023-07-01,2023-08-31,30,50,4326,smw,YOUR_GCP_PROJECT_ID
...
def process_image(
    ...
    if should_export(valid_pixel_ratio, config['valid_pixel_threshold']):
        ...
        export_to_drive(image, description, roi, 30, crs)
...
def main():
    ...
    try:
        ...
        # ROI読み込み
        roi = load_roi_from_shapefile_jp(config['roi_shapefile_path'])
