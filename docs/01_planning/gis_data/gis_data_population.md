...
密度（人/km²）でも同じ指標を算出しているが、密度は同一行では面積で割る単調変換であるため相関はほぼ同値（Pearson 0.8130 / Spearman 0.6921）になる。分析で用いる単位での値はサマリーJSONの`density_agreement`を参照。
...
| LandScan基準グリッド（ROIマスク後） | 1.012 | 集約後にROI外セルへ落ちたWorldPop画素（約45,000人）が除かれる |
