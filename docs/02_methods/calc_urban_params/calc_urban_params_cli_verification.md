...
**個別セル単位の独立検算**: 上記はいずれもROI全体の統計・分布での確認であり、個々のセルの値が元ラスタと対応しているかは別途確認が必要である（レビュー時の指摘）。GLC 30mの1セル（`cell_id=7598001935`、都心近郊、`LULC_BUILT_COV`が中間的な値を持つセルを選定）について、2種類の独立した経路で検算した。

1. **元ラスタからの独立画素カウント**: `rasterio` でセル範囲を直接切り出し、`build_class_lookup()` で写像してクラスごとに画素を数えた（算出パイプラインを経由しない完全に独立な実装）。結果は水域14.4%・農地56.1%・市街地29.5%となり、本番出力（水域17.7%・農地51.8%・市街地30.5%）との差は3〜4ポイントに達した。原因を調べたところ、GLCの画素（約28.0m×29.8m）は300mグリッドの正確な約数でないため、**手動で切り出した矩形はセル境界をまたぐ画素を完全カウントしてしまい**、`Resampling.average` による実際の面積按分とはズレる。これは検算方法自体の限界であり、実装の誤りではない
2. **同一集約関数を対象セル1つだけのGridSpecで独立再実行**: `aggregate_class_fractions_to_grid()` を、対象セルの範囲だけを覆う1x1の `GridSpec` で呼び出し、本番実行（38,235セル一括処理）の出力と突き合わせた。差は水域0.14ポイント・農地0.32ポイント・市街地0.18ポイントで、樹林・草地低木・湿地・裸地は完全一致した。差は浮動小数点の丸め・グリッド原点の微差の範囲に収まる

2つの検算を合わせて、集約アルゴリズムが入力ラスタを正しく反映していること、および手法1のズレが実装ではなく検算方法（矩形切り出しの境界処理）に起因することを確認した。
