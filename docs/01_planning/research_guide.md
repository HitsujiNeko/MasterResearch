...
**第一に、都市構造パラメータの包括的評価**  
都市構造をNDVIやNDBIといった衛星由来指標に限定せず、**建物密度・道路密度・緑被率・人口密度など、都市を構成する複数要素**を「都市構造パラメータ」として体系的に定義し、LSTとの関係を同時に評価する。実際にどの要素を採用するかは [urban_structure_parameters.md](urban_structure_parameters.md) を正本とする。先行研究S2では著者自身が「NDVI/NDBIのみでは不十分」と指摘しており<sup>[2]</sup>、本研究では測量データ（DGN）とOSM等の公開データを組み合わせることで、この課題に対処する。
