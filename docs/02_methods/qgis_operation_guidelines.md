...
ROI固定の単発描画が安全なのは、描画対象がROI内に限られ、全件の同時描画が発生しないためである。**外れ値ジオメトリによりレイヤーのextentが破綻するデータ**でも、描画範囲をROIに固定すればROI内の実データのみが描画され、無害化できることを確認している。測量データでは`測量_DH`・`測量_TH`にX=499,999／Y=999,999付近のダミー座標が記録されている（[survey_gis_data_preparation_status.md](../03_results/survey_gis_data_preparation_status.md)の座標範囲表を参照）。
