...
解釈上の前提:
    ...
    - ラスタの矩形範囲を一部しか含まないセルでは ``LST_VALID_RATIO`` が実際の被覆より
      高くなり、範囲外のセルは ``0.0`` になる（``aggregate_valid_ratio_to_grid()`` の
      既知の制約）。LSTラスタはROIでクリップ済みのため、影響を受けるのは解析BBox
      最外周のセルである（**FABDEMと範囲が一致するかは未確認**であり、両者を同一の
      有効域とみなしてはならない）。
    - 有効画素が1つも無い場合はケルビン検知（中央値判定）ができない。その場合は
      ``params.raster`` の共通処理が、グリッドと重なっていないのか、重なってはいるが
      全面欠測なのかを切り分けて警告する（標高と同じ規約）。
"""
...
def compute(resource: RasterResource, grid_spec: GridSpec) -> dict[str, np.ndarray]:
    ...
    Returns:
        ...
        （0-1、ラスタ範囲外は ``0.0``）。
    """
    # 全セルNaNだった場合の警告（重なり無し／全面欠測の切り分け）は共通処理が行う。
    ...
    # 有効値が1件も無い場合は判定材料が無いため、検知自体を行わない。
    lst_mean = columns["LST"]
